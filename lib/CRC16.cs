...
namespace sulfur_nitrogen_ac.lib
{
    /// <summary>
    /// 用于处理CRC数据校验工具类，来源于原来的上位机软件
    /// </summary>
    class CRC16
    {
        /// <summary>
        /// 多项式选择表
        /// </summary>
        static int[] wCRC16Table = { 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301,
        ...
        /// 生成CRC校验码
        /// </summary>
        /// <param name="pDataIn">字节数组</param>
        /// <param name="iLenIn">数组长度</param>
        /// <returns></returns>
        public static byte[] generateCRC16(byte[] pDataIn, int iLenIn)
        {
            int wResult = 0;
            ...
            bytes[0] = (byte)((wResult >> 8) & 0xFF);
            bytes[1] = (byte)((wResult) & 0xFF);
            return bytes;
        }
        /// <summary>
        /// 将crc结果双字节内容对调返回
        ...
        /// <returns></returns>
        public static byte[] generateCRC16_Ex(byte[] pDataIn, int iLenIn)
        {
            int wResult = 0;
            byte[] bytes = new byte[2];
            //short w = 0;
            ...
            bytes[1] = (byte)((wResult >> 8) & 0xFF);
            bytes[0] = (byte)((wResult) & 0xFF);
            return bytes;
        }
        /// <summary>
        /// 返回1字节异或校验结果
        ...
        /// <returns>1字节校验结果</returns>
        public static byte generateCRCXor(byte[] data)
        {
            byte CheckCode = 0;
            int len = data.Length;
            for (int i = 0; i < len; i++)
        ...
        public static void main(String[] args)
        {
            ...
            byte[] bb = { 0x05, 0x02, 0x00, 0x00, 0x00, 0x00 };
            byte ba = generateCRCXor(bb);
        }
